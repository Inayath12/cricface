...
const Catalog = () => {
  ...
  // Dynamically derived willow types from actual products
  const willowTypes = Array.from(
    new Set(products.map((p) => p.willow_type).filter(Boolean))
  );
  ...
  const getOriginalPrice = (product: Product) => {
    if (currency === 'INR') return product.original_price_inr;
    if (currency === 'USD') return product.original_price_usd;
    return product.original_price_eur;
  };
        ...
        <div className="mb-12">
          ...
          <div className="flex flex-col md:flex-row gap-4 items-center bg-white p-4 rounded-2xl shadow-sm border border-zinc-100 sticky top-24 z-30">
            ...
            <div className="flex gap-2 w-full md:w-auto">
              ...
              >
                <option value="">All Willow Types</option>
                {/* ✅ Dynamically generated from actual products in DB */}
              ...
              return (
                ...
                >
                  ...
                  <div className="p-6">
                    ...
                    <div className="flex items-center justify-between mt-auto pt-4 border-t border-zinc-50">
                      {/* ✅ Strikethrough original price + selling price */}
                      <div className="flex flex-col">
                        {hasDiscount && (
                          <span className="text-xs text-zinc-400 line-through">
                            {formatPrice({
                              price_inr: product.original_price_inr ?? product.price_inr,
                              price_usd: product.original_price_usd ?? product.price_usd,
                              price_eur: product.original_price_eur ?? product.price_eur,
                            })}
                          </span>
                        )}
                        <span className="text-xl font-bold text-cricket-green">
